...
class Resize(CustomTransform):
    ...
    def __call__(self, sample):
        ...
        binary_label = cv2.resize(binary_label, self.size, interpolation=cv2.INTER_NEAREST)
        instance_label = cv2.resize(instance_label, self.size, interpolation=cv2.INTER_NEAREST)
        _sample = sample.copy()
        _sample['img'] = img
        _sample['binary_label'] = binary_label
        _sample['instance_label'] = instance_label
        return _sample
...
class Rotation(CustomTransform):
    ...
    def __call__(self, sample):
        ...
        binary_label = cv2.warpAffine(binary_label, R, (binary_label.shape[1], binary_label.shape[0]), flags=cv2.INTER_NEAREST)
        instance_label = cv2.warpAffine(instance_label, R, (instance_label.shape[1], instance_label.shape[0]),
...
class ToTensor(CustomTransform):
    ...
    def __call__(self, sample):
        ...
        binary_label = torch.from_numpy(binary_label).type(torch.long)
        instance_label = torch.from_numpy(instance_label).type(torch.long)
        _sample = sample.copy()
        _sample['img'] = img
        _sample['binary_label'] = binary_label
        _sample['instance_label'] = instance_label
